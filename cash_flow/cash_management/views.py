...
class CashTransactionViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        ...
        if self.action == 'partial_update':
            ...
            return CashTransactionValidationSerializer
        ...
        if serializer.is_valid():
            try:
                with transaction.atomic():
                    expense_sheet = CashTransaction.create_expense_sheet(
            ...
            # )
            return Response(
                CashTransactionListingSerializer(expense_sheet).data,
    ...
    def update(self, request, pk=None, *args, **kwargs):
        ...
        if serializer.is_valid(raise_exception=True):
            try:
                with transaction.atomic():
                    expense_sheet = CashTransaction.update_expense_sheet(
                        cls = self,
                        expense_sheet_id = pk,
            ...
            headers = self.get_success_headers(serializer.data)
            return Response(
                CashTransactionDetailSerializer(expense_sheet).data,
    ...
    def partial_update(self, request, pk=None, *args, **kwargs):
        ...
        if serializer.is_valid(raise_exception=True):
            try:
                with transaction.atomic():
                    expense_sheet = CashTransaction.validate_expense_sheet(
                        expense_sheet_id=pk,
    ...
    def rejection(self, request, pk=None, *args, **kwargs):
        ...
        if serializer.is_valid(raise_exception=True):
            try:
                with transaction.atomic():
                    expense_sheet = CashTransaction.rejection_expense_sheet(
                        expense_sheet_id=pk,
                        observation=serializer.validated_data['description'],
                        user=request.infoUser.get('id')
                    )
    ...
    def destroy(self, request, pk=None):
        ...
        print(f"{user_qs}")
        # CashTransaction.delete_expense_sheet(self, user=user_qs.first)
        CashTransaction.delete_expense_sheet(user=user_qs, expense_sheet_id = pk)
        CashTransaction = self.get_object()
        return Response(
    ...
    def restore(self, request, pk=None, ):
        ...
        CashTransaction.restore_expense_sheet(user=user_qs, expense_sheet_id = pk)
        CashTransaction = self.get_object()
        return Response(
            CashTransactionDetailSerializer(
                CashTransaction,
